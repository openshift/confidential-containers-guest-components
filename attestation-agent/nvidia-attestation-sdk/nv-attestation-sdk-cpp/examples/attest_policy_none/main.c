...
int main(void) {
    ...
    nvat_rc_t err = nvat_attestation_ctx_set_verifier_type(ctx, NVAT_VERIFY_LOCAL);
    if (err != NVAT_RC_OK) {
    ...
    // Explicitly NO policy set (default behavior)
    ...
    if (rc == NVAT_RC_OVERALL_RESULT_FALSE) return 1;
    return 1;
}
