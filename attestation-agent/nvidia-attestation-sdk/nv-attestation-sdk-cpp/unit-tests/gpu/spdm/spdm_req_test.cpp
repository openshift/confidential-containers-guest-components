 ...
 */
class GpuSpdmReqTest : public ::testing::Test {
protected:
    /**
    ...
    void SetUp() override {
        ...
        std::ifstream json_file("testdata/spdm_req_parsed_output.json");
        if (!json_file.is_open()) {
            GTEST_FAIL() << "Failed to open unit-tests/testdata/spdm_req_parsed_output.json in SetUp";
        }
 ...
 *
 * The expected values are loaded from `spdm_req_parsed_output.json` which contains
 * the expected parsed fields of an SPDM GET_MEASUREMENTS request.
 */
TEST_F(GpuSpdmReqTest, ParseAndVerifySpdmRequest) {
    ASSERT_FALSE(m_expected_values_json.is_null()) << "Expected values JSON (spdm_req_parsed_output.json) is null. SetUp might have failed.";
